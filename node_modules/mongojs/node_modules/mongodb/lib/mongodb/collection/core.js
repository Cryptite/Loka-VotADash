...
var insertWithWriteCommands = function (self, docs, options, callback) {
    ...
    if (docs.length == 1) {
        ...
        var write_command = {
            insert: namespace, writeConcern: errorOptions, ordered: !continueOnError, documents: docs
        }

        // Execute the write command
        return self.db.command(write_command
            , { connection: connection, checkKeys: typeof options.checkKeys == 'boolean' ? options.checkKeys : true, serializeFunctions: serializeFunctions, writeCommand: true }
            , function (err, result) {
...
var removeWithWriteCommands = function (self, selector, options, callback) {
    ...
    self.db.command(write_command
        , { connection: connection, checkKeys: false, serializeFunctions: serializeFunctions, writeCommand: true }
        , function (err, result) {
            if (errorOptions.w == 0 && typeof callback == 'function') return callback(null, null);
            if (errorOptions.w == 0) return;
            if (callback == null) return;
...
var updateWithWriteCommands = function (self, selector, document, options, callback) {
    ...
    // Execute the write command
    self.db.command(write_command
        , { connection: connection, checkKeys: checkKeys, serializeFunctions: serializeFunctions, writeCommand: true }
        , function (err, result) {
            if (errorOptions.w == 0 && typeof callback == 'function') return callback(null, null);
            if (errorOptions.w == 0) return;
            if (callback == null) return;
...
var findAndModify = function findAndModify(query, sort, doc, options, callback) {
    ...
    var queryObject = {
        'findandmodify': this.collectionName, 'query': query
    };
