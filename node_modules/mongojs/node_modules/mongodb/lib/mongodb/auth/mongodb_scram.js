...
var authenticate = function (db, username, password, authdb, options, callback) {
    ...
    while (connections.length > 0) {
        ...
        var executeScram = function (connection) {
            ...
            var cmd = {
                saslStart: 1, mechanism: 'SCRAM-SHA-1', payload: new Binary(f("n,,%s", firstBare)), autoAuthorize: 1
            }
            ...
            db.db(authdb).command(cmd, { connection: connection }, function (err, r) {
                ...
                var cmd = {
                    saslContinue: 1, conversationId: r.conversationId, payload: new Binary(new Buffer(clientFinal))
                }
                ...
                db.db(authdb).command(cmd, { connection: connection }, function (err, r) {
                    if (r.done == false) {
                        var cmd = {
                            saslContinue: 1, conversationId: r.conversationId, payload: new Buffer(0)
                        }
